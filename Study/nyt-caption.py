...
df = pd.read_csv("D:/data/sample/ArticlesApril2017.csv")
print(df.columns)
...
class TextGeneration(Dataset):
    ...
    def __init__(self):
        ...
        for filename in glob.glob("D:/data/sample/*.csv"):
            if 'Articles' in filename:
                article_df = pd.read_csv(filename)

                all_headline.extend(list(article_df.headline.values))
                break

        all_headline = [h for h in all_headline if h!="Unknown"]

        self.corpus = [self.clear_text(x) for x in all_headline]
        self.BOW = {}
